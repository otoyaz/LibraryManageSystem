  ...
  <div id="HelloWorld">
    <header>
      <el-row>
          ...
          <el-col :span="20" class="title"><b>五邑大学图书馆</b></el-col>
    ...
    <article>
      <el-row class="aa">
        <el-radio-group v-model="isCollapse" style="margin:10px">
          ...
          <el-col :span="4">
            <el-menu router default-active="2" class="el-menu-vertical-demo" text-color="#5F5F60" :collapse="isCollapse">
   ...
   <footer>
    <p class="copy">Copyright © 2020.&nbsp;&nbsp;South China Institute Of Software.&nbsp; All Rights Reserved</p>
</footer>
 </div>
</template>
  ...
  computed: {
    ...
    methods: {
      handleCommand(command) {
        if(command == 'loginout'){
          let params = {readerId: sessionStorage.getItem("userId")};
          console.log(params);
          // return;
           this.$http.post('http://localhost:8081/logout',qs.stringify(params)).then((data)=>{
          }).catch((error)=>{
          })
          this.$router.push('/');
